...
// Works for 2D and ND: leading dims are treated as batch, last two are [M,K]×[K,N]→[M,N].
...
// input  [N, C_in, H, W]
// weight [C_in*kH*kW, C_out]  (caller passes transpose(W) where layer W is [C_out, C_in*kH*kW])
