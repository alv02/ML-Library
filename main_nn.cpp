...
int main() {
    ...
    // Flatten [N,3,32,32] → [N,3072] for dense layers
    u32 flat_train[2] = {val_X->shape[0], 3072};
    u32 flat_test[2]  = {test_val_X->shape[0], 3072};
    ...
    // Reshape back to [N,3,32,32] for visualization
    u32 vis_train[4] = {val_X->shape[0], 3, 32, 32};
    u32 vis_test[4]  = {test_val_X->shape[0], 3, 32, 32};
    tensor_reshape(val_X, vis_train, 4);
    tensor_reshape(test_val_X, vis_test, 4);
