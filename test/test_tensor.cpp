...
static void test_mat_mul_batched(const char *name, const char *dir,
    ...
    if (trans_a)
        tensor_transpose(a, a->ndim - 2, a->ndim - 1);
    if (trans_b)
        tensor_transpose(b, b->ndim - 2, b->ndim - 1);
...
int main(int argc, char **argv) {
    ...
    fflush(stdout);
    test_mat_mul_batched("3D             [2,3,4]@[2,4,5]→[2,3,5]",
                         "matmul_batched_3d");
    test_mat_mul_batched("3D trans_b     [2,3,4]@[2,5,4]^T→[2,3,5]",
                         "matmul_batched_3d_transB", false, true);
    test_mat_mul_batched("3D trans_a     [2,4,3]^T@[2,4,5]→[2,3,5]",
                         "matmul_batched_3d_transA", true, false);
    test_mat_mul_batched("4D             [2,3,4,5]@[2,3,5,6]→[2,3,4,6]",
                         "matmul_batched_4d");
    ...
    printf("\n-- maxpool2d forward --\n");
    fflush(stdout);
    test_maxpool2d("1×1×4×4 k=2 s=2 p=0 → [1,1,2,2]", "maxpool_1c_4x4_k2s2",
                   Unfold2dParams(2, 2, 0));
    test_maxpool2d("1×2×4×4 k=2 s=2 p=0 → [1,2,2,2]", "maxpool_2c_4x4_k2s2",
                   Unfold2dParams(2, 2, 0));
    test_maxpool2d("2×3×6×6 k=2 s=2 p=0 → [2,3,3,3]", "maxpool_2n3c_6x6_k2s2",
                   Unfold2dParams(2, 2, 0));
    test_maxpool2d("2×4×6×6 k=3 s=1 p=0 → [2,4,4,4]", "maxpool_2n4c_6x6_k3s1",
                   Unfold2dParams(3, 1, 0));
    ...
    test_welford_mean_var("4D   dim=1 (8,16,8,8)→(16)", "welford_4d_8n16c_8x8",
                          1);
    test_welford_mean_var("4D   dim=1 (16,32,16,16)→(32)",
                          "welford_4d_16n32c_16x16", 1);
